...
const callback = (entries, observer) => {
  entries.forEach(entry => {
    if (entry.isIntersecting) {
      // Quando o elemento aparecer na tela, adicione a classe 'nova-classe'
      entry.target.classList.add('animate__animated');
      entry.target.classList.add('animate__fadeInLeft');
    } else {
      // Quando o elemento sair da tela, remova a classe 'nova-classe'
...
const options = {
  root: null, // Usa o viewport como o container de visualização
};

// Cria um novo observer
const observer = new IntersectionObserver(callback, options);

// Seleciona todos os elementos que deseja observar
const elementosParaObservar = document.querySelectorAll('.info-box-01, .info-box-02, .info-box-03, .heading-01, .promo-row, .promo-slider-cont, .brands-mobile, .conditions-cont, .tips.paragraph, .tips-box, .tips-slider-cont, .content-blog, .paragraph-01, .faq-cont');
